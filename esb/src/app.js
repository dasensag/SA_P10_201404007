...
app.get('/order/:orderId', (req, res) => {
    axios.get('http://restaurant:4000/order/' + req.params.orderId).then(function (response) {
...
app.post('/order', jsonParser, function (req, res) {
    ...
    axios.post('http://restaurant:4000/order', req.body, {
        headers: {
...
app.get('/delivery/:orderId', (req, res) => {
    axios.get('http://delivery:5000/delivery/' + req.params.orderId).then(function (response) {
        log.info('GET /order/', req.params.orderId, ' ', response.data);
        res.json(response.data);
    });
})
...
app.post('/delivery', jsonParser, (req, res) => {
    axios.post('http://delivery:5000/delivery', req.body, {
        headers: {
            'Content-Type': 'application/json'
        }
    }).then(function (response) {
